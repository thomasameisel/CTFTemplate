...
function firstRun() {
  ...
  db.run('CREATE TABLE IF NOT EXISTS challenges (challenge_name TEXT, points INT, challenge_content TEXT)'); // ROWID is primary key
  db.run('CREATE TABLE IF NOT EXISTS flags (challenge_id INT, flag TEXT, PRIMARY KEY(challenge_id, flag))');
  ...
  db.run('CREATE VIEW IF NOT EXISTS all_users_completed AS SELECT users.username AS username, challenge_name, points, attempt_time FROM users NATURAL JOIN (attempts JOIN challenges ON attempts.challenge_id=challenges.ROWID) WHERE attempts.correct=1');
  ...
  db.run('CREATE VIEW IF NOT EXISTS challenges_flags AS SELECT challenges.ROWID as challenge_id, challenge_name, challenge_content, points, flag FROM challenges, flags WHERE challenges.ROWID=flags.challenge_id');
